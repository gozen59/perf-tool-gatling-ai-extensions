...
When the **gatling** MCP server is configured in Cursor, use its tools for Gatling Enterprise API operations when they help the workflow (listing resources, validating account data, etc.).
...
1. Verify `.gatling/package.conf` exists and is not empty.
   If missing or empty, help the user create it using the skill **gatling-configuration-as-code** (`.cursor/skills/gatling-configuration-as-code`).
